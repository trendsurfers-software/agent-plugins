...
3. Point StrategyLens at PortfolioManager's **AI reports folder**. Newer PortfolioManager builds do this for you: they write the `PM_SL_REPORTS_ROOT` environment variable automatically whenever the MCP server is on, and the plugin reads it — nothing to configure. On older builds, set `SL_REPORTS_ROOT` yourself to the exact path shown in PM Settings → MCP Server (absolute path; the directory must exist). Sharing this one folder is what makes the combo run smoothly: PortfolioManager writes every backtest report into it, and StrategyLens is allowed to read exactly it — reports are analyzable the moment a backtest finishes, with no copying and no permission errors.
   - StrategyLens can be allowed **more than one folder** if you need it (say, an archive of older reports). Claude Code: set `SL_REPORTS_ROOT` to a comma-separated list (`D:\Main Reports,E:\Archive`) — it works alongside the PM-managed variable, and the same folder appearing in both counts once. Codex: add each extra folder as one more path at the end of the `args` list in the `config.toml` block below.
...
2. Only needed on older PortfolioManager builds (newer ones maintain `PM_SL_REPORTS_ROOT` automatically): under *User variables*, click **New…** — Name: `SL_REPORTS_ROOT`, Value: the full path of PortfolioManager's AI-reports folder (shown in PM Settings → MCP Server). Click OK.
