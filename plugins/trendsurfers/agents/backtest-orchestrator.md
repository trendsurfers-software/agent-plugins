...
- `expertAdvisor` is required on every spec; get exact paths from `list_experts` and symbol names from `find_symbols` per broker. `endDate` behaves as EXCLUSIVE (measured: coverage consistently stops the day *before* the requested date, on fully historical dates too) — request one day past the last day you actually want covered, and verify actual coverage via `sl_analyze_reports`'s `identity.tradedRange` rather than trusting the requested date. Explicit `tickModel` on every spec — never rely on a default.
