...
- **Never hand-decode a report's raw HTML/UTF-16 — `sl_analyze_reports`'s `identity` section already has it.** `identity.eaInputs` returns the EA's full parsed parameter table as JSON (use it for any "did this input actually apply / was it corrupted" check — e.g. confirming a `parameterOverrides` or `autoRemapSymbols` run didn't mutate a field, or comparing against a frozen `.set`). `identity.tradedRange`/`testedRange` give exact first/last trade timestamps and the requested-vs-actual window (use them for any "did the window truncate / does this report's coverage match another's" check). Reaching for `python`/`Bash` UTF-16 parsing on a report means this step was skipped — call `sl_analyze_reports` with `sections: ["identity"]` first.
