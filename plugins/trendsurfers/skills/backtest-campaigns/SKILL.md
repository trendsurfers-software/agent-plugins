...
- After analysis, read the **`historyQualityPercent`** field from `sl_analyze_reports` for every report (HTML reports only — `.xlsx` exports carry no quality header, so the field is null there; the sibling `reportTickModel` field confirms the tick model actually recorded in the report). Quality below 90% marks the run `quality-degraded` in campaign.md — its stats are directional only, never a verdict. Quality wildly different between two compared runs voids the comparison.
...
4. `enqueue_backtests` with: a fresh `idempotencyKey` per logical call (reuse only to safely retry the identical request), `reportRoot` = the phase folder, `label` per the naming convention, `tickModel` explicit (never rely on a default inside a campaign), `periods` for multi-window specs (≤20 per spec), `includeBacktestImages: false` — images slow batches for no analytical benefit.
...
8. `endDate` is INCLUSIVE. Dates are `YYYY-MM-DD`.
9. Run one MT5 test driver at a time. If working across multiple sessions or plugin instances, space terminal launches at least 30 seconds apart to avoid startup collisions.
