...
- `PM_MCP_TOKEN` plus a reports root (`PM_SL_REPORTS_ROOT`, auto-written by newer PortfolioManager builds, or a manually set `SL_REPORTS_ROOT`) present in the environment, followed by a full client restart — environment variable changes are not picked up mid-session.
...
2. If a reports-root var (`PM_SL_REPORTS_ROOT` or `SL_REPORTS_ROOT`) is set and the directory is missing, create it. If neither is set, tell the user: newer PortfolioManager builds write `PM_SL_REPORTS_ROOT` automatically once MCP is enabled; otherwise set `SL_REPORTS_ROOT` to PortfolioManager's AI-reports folder (PM Settings → MCP Server). Then restart the client.
...
2. The effective reports root (`PM_SL_REPORTS_ROOT` if set, else `SL_REPORTS_ROOT`; the latter may hold several comma-separated folders) is absolute and each folder exists. FAIL fix: create it, or run the init procedure above.
